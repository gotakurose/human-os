...
- 16タイプ内の65主ルート・163サブルート
- 結果分岐ID
- 回答証拠1,514件
- 回答証拠の対象側
- 回答証拠重み算式
- 最低証拠数3
- 判定配点 `回答証拠55％ / 5能力30％ / 4軸15％`
- 主ルート・サブルート選出順序
- 同点処理
- 確信度閾値
- 数値補正上限
...
- URL・保存方式
- 実装後QA
- Preview・Production反映

### 正本の責任分担

| 正本 | 管理内容 |
|---|---|
| 質問・採点・判定仕様 | 4軸、16タイプ、5能力、V2分岐判定式 |
| 結果文章・生成仕様 | V1/V2結果構造、文章責任、数値補正 |
| 固定名称・IDマスター | typeId、4文字コード、結果分岐ID形式 |
| 結果分岐マスター.csv | 65主ルート・163サブルートの全対応 |
| 回答証拠マスター.csv | 1,514回答証拠と重み |
| 実装・QAチェックリスト | 実装順序と合格条件 |

同一内容を複数正本へ重複定義しない。

## 12. V2次工程

1. 実装用データ構造・JSONスキーマ設計
2. 163本文データの正本化・JSON変換
3. Claude Code実装指示
4. コード実装
5. 163代表回答・回帰・表示QA
6. Preview確認
