...
> 文書状態：正本・V1実装済み／V2結果分岐IDロック済み・未実装
> 更新日：2026年7月17日
> 対象：Human OS / ビジマル診断
...
V2の結果分岐IDは以下の形式で固定する。

```text
mainRouteId = {typeId}.mNN
subRouteId  = {typeId}.mNN.sNN
```
...
正確な対応表は `Human OS｜ビジマル診断｜結果分岐マスター.csv` を正とする。本書へ163件を複製しない。

結果分岐IDは内部管理用であり、結果画面・シェア文には表示しない。表示名を修正してもIDは変更しない。
